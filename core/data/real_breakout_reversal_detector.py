#!/usr/bin/env python3
"""
REAL BREAKOUT & REVERSAL DETECTOR
- Use real market data and actual DP-aware system
...
class RealBreakoutReversalDetector:
    ...
    async def run_real_breakout_reversal_detection(self, tickers: List[str], duration_hours: int = 4) -> Dict[str, Any]:
    ...
    async def _detect_real_breakout_reversal(self, ticker: str) -> Optional[Dict[str, Any]]:
        """Detect real breakout or reversal"""
        try:
            ...
            # Skip RapidAPI entirely and use yfinance directly
            logger.info(f"Using yfinance for {ticker} (RapidAPI rate limited)")
            ...
            if not hist.empty:
                ...
                try:
                    ...
                    if options:
                        ...
                        options_data = {
                            'options': [
                                {'type': 'call', 'volume': vol} for vol in option_chain.calls['volume']
                            ] + [
                                {'type': 'put', 'volume': vol} for vol in option_chain.puts['volume']
                            ]
                        }
    ...
    def _check_options_flow(self, options_data: Dict[str, Any], option_type: str) -> bool:
        ...
        try:
            ...
            # Check for significant volume in options
            total_volume = 0
            for option in options:
                if option.get('type') == option_type:
                    total_volume += option.get('volume', 0)
            
            return total_volume > self.options_flow_threshold
...
async def main():
    """Main function"""
    print("🔥 REAL BREAKOUT & REVERSAL DETECTOR")
    print("=" * 80)
